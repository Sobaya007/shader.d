...
@blockName = thread_local global %kernel.BlockName zeroinitializer, comdat, align 1 #3 ; [#uses = 2]
...
define void @fragMain() #4 comdat {
  ...
  %3 = getelementptr inbounds %kernel.BlockName, %kernel.BlockName* @blockName, i32 0, i32 1 ; [#uses = 1, type = i8*]
  %4 = load i8, i8* %3                            ; [#uses = 1]
  %5 = trunc i8 %4 to i1                          ; [#uses = 1]
  br i1 %5, label %if, label %endif

if:                                               ; preds = %0
  %6 = getelementptr inbounds %kernel.BlockName, %kernel.BlockName* @blockName, i32 0, i32 0 ; [#uses = 1, type = %kernel.S*]
  %7 = getelementptr inbounds %kernel.S, %kernel.S* %6, i32 0, i32 1 ; [#uses = 1, type = [5 x %"shader.builtin.Vector!(float, 4LU).Vector"]*]
  %8 = getelementptr inbounds [5 x %"shader.builtin.Vector!(float, 4LU).Vector"], [5 x %"shader.builtin.Vector!(float, 4LU).Vector"]* %7, i32 0, i64 2 ; [#uses = 1, type = %"shader.builtin.Vector!(float, 4LU).Vector"*]
  call void @_D6shader7builtin__T6VectorTfVmi4ZQo__T8opBinaryVAyaa1_2bZQtMFSQCjQCf__TQCaTfVmi4ZQCkZQy(%"shader.builtin.Vector!(float, 4LU).Vector"* noalias sret align 1 %.sret_tmp, %"shader.builtin.Vector!(float, 4LU).Vector"* nonnull @color1, %"shader.builtin.Vector!(float, 4LU).Vector"* byval align 1 %8) #7
  br label %endif
